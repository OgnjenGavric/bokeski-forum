...
/*	-------------------------------------------------------------------------
	...
	if(isset($_POST["sendermessage"])){
		if (strlen($sendermessage) < 4) {
			if (!$sendermessage) {
				$errors[] = "You must enter a message.";
			} else {
				$errors[] = "Message must be at least 4 characters.";
			}
		}
	}
	...
	if ($errors) {
		...
		echo '<div style="color:#005f84;font-size:32px;font-weight:bold;text-align:center;font-family:arial">Došlo je do sljedećih grešaka:<br><ul>'. $errortext .'</ul></div>';
		...
		$mail->Grad = $grad;
		$mail->Drzava = $drzava;
		$mail->Odrasli = $odrasli;
		$mail->Djeca = $djeca;
		$mail->Dolazak = $dolazak;
		$mail->Odlazak = $odlazak;
		...
		if($mail->Send()) {
			...
			/*	---------------------------------------------------------------------
				...
				if($redirectForm == true){
					echo '<script>setTimeout(function () { window.location.replace("'.$redirectForm_url.'") }, 2000); </script>';
				}
							
			  	echo '<div style="color:#005f84;font-size:32px;font-weight:bold;text-align:center;font-family:arial">Poruka je uspješno poslata!</div>';
				} 
				else {
				  echo '<div style="color:#005f84;font-size:32px;font-weight:bold;text-align:center;font-family:arial">Poruka nije poslata - došlo je do greške servera!</div>';	
				}
	}
?>